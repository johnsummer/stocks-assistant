...
- `lr yyyymmdd [本玉数]`：ロットサイズを既定のアルゴリズムで自動計算する。
...
- 基本的な計算式：玉サイズ=トレードで注文できる最大金額÷((銘柄ごとのヘッジ数+銘柄ごとの本玉数)×銘柄数×株価×1.1)
- トレードで注文できる総金額=トレードで導入する資金×3.3（信用取引を利用する前提）
  - 複数の注文方式（終値、翌日寄付、など）で同時にトレードしている場合、その中で一番小さい総資金を導入資金とする
- 1銘柄のトレードにおいて、ヘッジ：本玉＝1:1　※暫定、今後これも指定できるように改善したい
- 同時にトレードしていい銘柄数=2　※暫定、今後これも指定できるように改善したい
- 玉サイズは100の桁まで切り捨てる。
